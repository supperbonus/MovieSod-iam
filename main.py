...
def admin_account(id):
    ...
    if id:
        ...
        if request.method == 'POST' and 'submit' in request.form:
            ...
            if account['password'] != request.form['password']:
                ...
                hash = hashlib.sha1(hash.encode())
                password = hash.hexdigest()
            cursor.execute('UPDATE accounts SET username = %s, password = %s, email = %s , role = %s WHERE id = %s', (
                request.form['username'], password, request.form['email'], request.form['role'], id,))
            mysql.connection.commit()
    ...
    if request.method == 'POST' and request.form['submit']:
        ...
        hash = hashlib.sha1(hash.encode())
        password = hash.hexdigest()
        cursor.execute('INSERT INTO accounts (username,password,email,role) VALUES (%s,%s,%s,%s)', (
            request.form['username'], password, request.form['email'],request.form['role'],))
        mysql.connection.commit()
        return redirect(url_for('admin'))
